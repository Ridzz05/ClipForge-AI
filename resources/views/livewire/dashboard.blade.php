<div @if($poll) wire:poll.4s @endif class="grid" style="gap: 32px;">
    ...
    <div class="panel" style="padding: 18px 24px; background: var(--stage-2); border: 1px dashed rgba(245,239,228,0.15); border-radius: 18px; color: #f5efe4;">
        <div class="row between" style="gap: 16px; flex-wrap: wrap;">
            <!-- Left: Status Indicators -->
            <div class="row" style="gap: 20px; flex-wrap: wrap;">
                <div style="font-family: var(--mono); font-size: 11px; font-weight: 700; color: rgba(245,239,228,0.5); text-transform: uppercase; letter-spacing: 0.15em;">
                    Sistem Status:
                </div>
                ...
                <div class="row" style="gap: 8px; font-size: 12px; font-family: var(--mono); letter-spacing: 0.05em;">
                    ...
                    <span style="color: rgba(245,239,228,0.5)">Whisper:</span>
                    <strong style="color: {{ ($statuses['whisper'] ?? false) ? '#f5efe4' : '#ef4444' }};">{{ ($statuses['whisper'] ?? false) ? 'Online' : 'Offline' }}</strong>
                </div>
                ...
                <div class="row" style="gap: 8px; font-size: 12px; font-family: var(--mono); letter-spacing: 0.05em;">
                    ...
                    <span style="color: rgba(245,239,228,0.5)">Face Tracker:</span>
                    <strong style="color: {{ ($statuses['face'] ?? false) ? '#f5efe4' : '#ef4444' }};">{{ ($statuses['face'] ?? false) ? 'Online' : 'Offline' }}</strong>
                </div>
                ...
                <div class="row" style="gap: 8px; font-size: 12px; font-family: var(--mono); letter-spacing: 0.05em;">
                    ...
                    <span style="color: rgba(245,239,228,0.5)">LLM Scorer ({{ ucfirst($statuses['llm_driver'] ?? 'ollama') }}):</span>
                    <strong style="color: {{ ($statuses['llm'] ?? false) ? '#f5efe4' : '#ef4444' }};">{{ ($statuses['llm'] ?? false) ? 'Online' : 'Offline' }}</strong>
                </div>
            </div>

            <!-- Right: Restart Actions -->
            <div class="row" style="gap: 12px;">
                <button type="button" wire:click="restartQueue" class="btn btn-sm btn-outline" style="border-color: rgba(245,239,228,0.2); background: rgba(255,255,255,0.03); color: #f5efe4; display: flex; align-items: center; gap: 6px;" wire:loading.attr="disabled">
    ...
    <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 24px; align-items: stretch;">
        ...
        <div class="panel grid" style="gap: 16px; background: var(--tile-2); border-color: rgba(0,0,0,0.05); color: var(--ink);">
            <h3 style="margin: 0; font-family: var(--serif); font-style: italic; font-size: 24px; font-weight: 700; color: var(--ink);">Upload File Video</h3>
            ...
            <form wire:submit="save" class="grid" style="gap: 18px; justify-content: space-between; height: 100%;">
                ...
                <div style="border: 2px dashed rgba(26,23,20,0.15); border-radius: 18px; padding: 28px 20px; text-align: center; background: rgba(255,255,255,0.25); transition: border-color 0.2s ease;"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="color: var(--ink); margin-bottom: 12px;">
                ...
                <div class="row between" style="gap: 12px; margin-top: auto;">
                    <button type="submit" class="btn btn-primary" style="width: 100%; border-color: var(--ink); background: var(--ink); color: #f5efe4;"
        ...
        <div class="panel grid" style="gap: 16px; background: var(--tile-1); border-color: rgba(0,0,0,0.05); color: var(--ink);">
            <h3 style="margin: 0; font-family: var(--serif); font-style: italic; font-size: 24px; font-weight: 700; color: var(--ink);">Ambil Video dari URL</h3>
            ...
            <form wire:submit="ingestUrl" class="grid" style="gap: 20px; justify-content: space-between; height: 100%;">
                <div class="grid" style="gap: 12px;">
                    ...
                    <div style="border: 1.5px dashed rgba(26,23,20,0.15); border-radius: 12px; padding: 16px; background: rgba(255,255,255,0.25); display: flex; gap: 12px; align-items: flex-start; margin-top: 8px;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0; margin-top: 1px; color: var(--ink);">
                            ...
                            <tr wire:key="video-{{ $video->id }}">
                                ...
                                <td style="text-align: right;">
                                    @if(in_array($video->status, ['reviewing','done']))
                                        <a href="/videos/{{ $video->id }}/review" class="btn btn-sm btn-primary" style="padding: 5px 12px; border-color: var(--ink); background: var(--ink); color: #f5efe4;">Review Hasil</a>
                                    ...
                                    @else
                                        <span class="muted" style="font-size:12px; display: inline-flex; align-items: center; gap: 6px;">
                                            <span class="spin" style="width:10px;height:10px;border-width:1.5px;"></span> Proses&hellip;
                                        </span>
                                    @endif
                                </td>
                            </tr>
